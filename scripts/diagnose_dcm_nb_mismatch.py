#!/usr/bin/env python3
"""Diagnose JAX MIC vs PyCHARMM nonbonded mismatch for a DCM liquid PBC box.

Focus: inter-monomer VDW (analog of water_water in the tri-alanine box) without
selective CHARMM BLOCK (unsafe under mpirun).
...
Examples (CHARMM node)::
    ...
    ./scripts/mmml-charmm-mpirun.sh python scripts/diagnose_dcm_nb_mismatch.py \\
      -o artifacts/dcm_nb_parity --n-monomers 52 --box-side-A 28 --perturb-seed 31
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="DCM liquid box: JAX MIC vs PyCHARMM parity (inter-monomer VDW)",
    )
    parser.add_argument(
        "-o",
        "--output-dir",
        type=Path,
        default=Path("artifacts/dcm_nb_parity"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=31,
        help="Gaussian coordinate perturbation seed",
    )
    parser.add_argument("--n-monomers", type=int, default=10)
    ...
    parser.add_argument(
        "--skip-switch-audit",
        action="store_true",
        help="Skip JAX fswitch/vfswitch derivative self-check",
    )
    parser.add_argument(
        "--switch-audit-top-k",
        type=int,
...
def main() -> int:
    ...
    if args.no_build:
        coords_path = dcm_liquid_box_coords_path(workdir)
        if coords_path is None:
            print(
                f"Workdir {workdir} missing PSF/coords. Run once without --no-build.",
                file=sys.stderr,
            )
            return 2
        print(f"Reloading box from {workdir} ({coords_path.name})...", flush=True)
        box = reload_dcm_liquid_box_in_charmm(
            workdir,
        ...
        print(
            f"Building {args.n_monomers}× DCM liquid box in CHARMM "
            f"({args.box_side_A:.1f} Å cube)...",
            flush=True,
        )
        box = build_dcm_liquid_box_in_charmm(
            n_monomers=args.n_monomers,
            box_side_A=args.box_side_A,
            seed=args.seed,
            workdir=workdir,
