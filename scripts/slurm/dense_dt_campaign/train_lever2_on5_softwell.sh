#!/usr/bin/env bash
# Soft-well E_int aux FT at mm_switch_on=5 (freeze LJ scales, lr=1e-4).
#
#   bash scripts/slurm/dense_dt_campaign/submit_train_lever2_on5_softwell.sh
set -euo pipefail
...
SCALE_SIDECAR="${DDC_ON5SW_SCALE_SIDECAR:-artifacts/lj_scales/ckpts/hybrid_mm_fixed_lj_scales-4d68132d-c686-4ded-9887-efc16d5b2638/hybrid_mm.json}"
CKPT_DIR="${DDC_ON5SW_CKPT_DIR:-artifacts/lj_scales/ckpts}"
TAG="${DDC_ON5SW_TAG:-hybrid_mm_lever2_on5_softwell}"
EPOCHS="${DDC_ON5SW_EPOCHS:-20}"
N_TRAIN="${DDC_ON5SW_N_TRAIN:-32000}"
N_VALID="${DDC_ON5SW_N_VALID:-5950}"
SEED="${DDC_ON5SW_SEED:-42}"
BATCH="${DDC_ON5SW_BATCH:-64}"
LR="${DDC_ON5SW_LR:-0.0001}"
SW_STEPS="${DDC_ON5SW_STEPS:-64}"
SW_EVERY="${DDC_ON5SW_EVERY:-20}"
SW_BATCH="${DDC_ON5SW_SW_BATCH:-32}"
SW_SCALE="${DDC_ON5SW_LOSS_SCALE:-50.0}"
...
for f in "$CONFIG" "$DATA" "$CKPT" "$SCALE_SIDECAR"; do
  [[ -f "$f" ]] || { echo "ERROR: missing $f" >&2; exit 2; }
done
[[ -n "${CUDA_VISIBLE_DEVICES:-}" || -n "${SLURM_JOB_ID:-}" ]] || {
  echo "ERROR: submit via submit_train_lever2_on5_softwell.sh (need GPU allocation)" >&2
  exit 2
}

echo "=== lever-2 on=5 SOFT-WELL E_int AUX FT ==="
...
uv run mmml physnet-train \
  ...
  --soft-well-target-hi -3.0 \
  --frozen-mm-lj-scales-sidecar "$SCALE_SIDECAR" \
...
for k in (
    ...
    if k in a:
        b[k] = a[k]
b["learn_mm_lj_scales"] = True
b["mm_switch_on"] = 5.0
b["ml_switch_width"] = 1.5
b["mm_switch_width"] = 5.0
b["scales_source"] = str(src)
b["ft_recipe"] = "softwell_e_int_aux_on5_freeze_lj_lr1e-4"
...
echo "  next    : bash scripts/slurm/dense_dt_campaign/submit_eval_lever2_on5_softwell.sh"
date -Is
