# Menshutkin reaction — free-energy profiles
...
`jp1c09710.pdf` here. Also `truong1997.pdf` (GCOSMO continuum), which supplies
...
| document | contents |
|---|---|
| [`RESULTS.md`](RESULTS.md) | **the findings** — barriers, solvent effects, ion-pair states, limits |
| [`SUBMIT.md`](SUBMIT.md) | **how to run each stage** — prerequisites, one command, how to verify |
| [`HANDBOOK.md`](HANDBOOK.md) | operating notes: machines, traps, diagnostics, what is settled |
| [`ROADMAP.md`](ROADMAP.md) | what comes next and in what order |
| this file | what the campaign is, and the physics you must not break |
...
and −15.2. The earlier campaign gave −8.4 and put water and acetonitrile 0.07
kcal/mol apart; both were artifacts of running with `stop_gradient` on the ML
charges, which changes the force by 101 % against finite differences. See
[`RESULTS.md`](RESULTS.md) §1 and task #22.
...
This file used to say the opposite: that it "must come off past the ion pair"
because the training angle falls to 85–88° once the chloride is far. **That was
read off the wrong file.** The two training sources disagree, and both are
...
heat stage. The 100 ps equilibrated box is cached per (solvent, box, seed).
...
and is what every result in [`RESULTS.md`](RESULTS.md) uses. The historical note
that it "runs away (q(Cl) −0.80 → −1.03 in 50 fs)" came from a probe seeded at
the wrong ξ — it restrained a box equilibrated at ξ ≈ 0 to ξ = +0.50 against a
...
**`mechanical-fluct` forces are NOT the gradient of its energy.** This file used
to claim they were — "because there is no dq/dR term to drop" — which is true of
`mechanical` (charges genuinely frozen, dq/dR identically zero) and false of
`mechanical-fluct`, where q depends on R. Measured on an acetonitrile box at
ξ = +0.30, analytic gradient against central finite differences of the *same*
energy function:
...
`charge_gradient_scale` (λ) allows a damped response: `q_eff = λq + (1−λ)·sg(q)`
leaves the value exactly q and scales the gradient to λ·dq/dR. Untested, and no
longer needed — undamped dq/dR is stable in production.

> **Fixed 2026-08-02.** `run_solvated_production.sh` used to add
> `--freeze-charge-forces` automatically whenever `EMB=electrostatic`, which set
...
Adds `ml_mm_pol`: `E = −½ Σ αᵢ|Eᵢ|²`, the induced component EMLE
(J. Chem. Theory Comput. 2023, 19, 1417) obtains from a Thole model. Its point
is that this needs **no QM/MM training data** — in-vacuo atomic properties
suffice.

> **Out of scope for this campaign.** Run as a controlled A/B, polarisation
> *raises* the barrier (+1.54 water, +0.09 acetonitrile) and inverts the solvent
> ordering — away from Turan and from experiment. Since dq/dR alone reproduces
> the reference, and since Turan's MS-ARMD is itself non-polarisable so
> dq/dR-only is the like-for-like comparison, the polarisation runs were stopped.
> Kept here because the term is implemented, tested, and worth revisiting when
> the target is experiment rather than Turan. See [`RESULTS.md`](RESULTS.md) §4.
...
potentials are "trained to reproduce the energies of molecular systems as a
whole, without considering the response to external electric fields, making it
impossible to combine ML with MM in an electrostatic embedding scheme unless the
architecture of the ML model is modified."
