...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "mmml/data/new-dcm-round-2-only_MP2_41950.npz."
        ),
    )
    parser.add_argument(
        "--checkpoint",
...
def resolve_dataset_path(arg: Path | None) -> Path:
    candidate = arg or Path(os.environ.get("MMML_DATA", "mmml/data/new-dcm-round-2-only_MP2_41950.npz"))#mmml/data/fixed-acetone-only_MP2_21000.npz"))
...
def main() -> int:
    ...
    setupRes.generate_residue("DCM DCM")
    ic.build()
    coor.show()
    ...
    trajOpt = ase_io.Trajectory(f'{args.output}_opt_BFGS0physnet.traj', 'w')

    # minimize in ase
    print("Minimizing in ase")
    import ase.optimize as ase_opt
    print("starting minimization")
    _ = ase_opt.BFGS(ml_atoms,trajectory=trajOpt).run(fmax=0.01, steps=100)
    trajOpt = ase_io.Trajectory(f'{args.output}_opt_BFGS0hybrid.traj', 'w')
    print("minimization done")
    print("starting minimization")
    _ = ase_opt.BFGS(atoms, trajectory=trajOpt).run(fmax=0.01, steps=100)
    print("minimization done")
    ...
    if args.pdbfile:
        ...
        print(coor.show())
        params, model = load_model_parameters(epoch_dir, 100)
        model.natoms = 100
        print(model)
        calculator_factory = setup_calculator(
            ATOMS_PER_MONOMER=5,
        ...
        hybrid_energy = float(atoms.get_potential_energy())
        hybrid_forces = np.asarray(atoms.get_forces())
        trajOpt = ase_io.Trajectory(f'{args.output}_opt_BFGS1.traj', 'w')
        print("Minimizing in hybrid calculator")
        _ = ase_opt.BFGS(atoms, trajectory=trajOpt).run(fmax=0.0001,  steps=100)
        ...
        minimize.run_abnr(nstep=1000, tolenr=1e-6, tolgrd=1e-6)
        pycharmm.lingo.charmm_script("ENER")
        #print(coor.show())
        
        trajOpt = ase_io.Trajectory(f'{args.output}_opt_BFGS2.traj', 'w')

        ase_atoms.set_positions(coor.get_positions())
        _ = ase_opt.BFGS(atoms, trajectory=trajOpt).run(fmax=0.001, steps=100)
        ...
        for i in range(num_steps):
            ...
            if kinetic_energy[i] > 20:
                ...
                minimize.run_abnr(nstep=1000, tolenr=1e-6, tolgrd=1e-6)
                pycharmm.lingo.charmm_script("ENER")
                trajOpt = ase_io.Trajectory(f'{args.output}_opt_BFGS{i+2}.traj', 'w')
                ase_atoms.set_positions(coor.get_positions())
                _ = ase_opt.BFGS(atoms, trajectory=trajOpt).run(fmax=0.0001, steps=100)
                MaxwellBoltzmannDistribution(ase_atoms, temperature_K=temperature)
                Stationary(ase_atoms)  # Remove center of mass translation.
