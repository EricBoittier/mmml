...
Current-tree counts in this document are generated by
`scripts/generate_package_architecture.py` and checked in CI.
`[evidence: package_architecture_counts]` The dependency arrows remain an
architectural overview rather than a complete import graph.
...
Legacy import aliases may remain for checkpoint compatibility, but maintained
code should use the canonical paths above.
