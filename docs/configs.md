...
Preset composition rules live next to the presets themselves, in
`mmml/cli/run/presets/README.md` (outside the docs tree, so it is not linked
here — MkDocs builds in strict mode and only resolves links within `docs/`).
