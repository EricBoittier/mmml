...
```text
usage: mmml train [-h] [--config CONFIG] [--save-config SAVE_CONFIG] [--model {dcmnet,physnetjax}] [--physnet-checkpoint PHYSNET_CHECKPOINT] [--physnet-transfer-model PHYSNET_TRANSFER_MODEL] [--list-physnet-transfer-models]
                  [--physnet-transfer-category PHYSNET_TRANSFER_CATEGORY] [--train TRAIN] [--valid VALID] [--train-fraction TRAIN_FRACTION] [--batch-size BATCH_SIZE] [--max-epochs MAX_EPOCHS] [--learning-rate LEARNING_RATE] [--early-stopping EARLY_STOPPING]
                  [--targets TARGETS [TARGETS ...]] [--output OUTPUT] [--log-interval LOG_INTERVAL] [--center-coords] [--normalize-energy] [--rot-augment] [--rot-perturbation ROT_PERTURBATION] [--verbose] [--quiet] [--dry-run]
...
options:
  ...
  --physnet-transfer-model PHYSNET_TRANSFER_MODEL
                        Bundled PhysNet transfer model ID, file stem, or category. Defaults to the 'joint-training-defaults' charged model for physnetjax training.
  ...
  --train-fraction TRAIN_FRACTION
                        Fraction for train split if --valid not provided (default: 0.8)
