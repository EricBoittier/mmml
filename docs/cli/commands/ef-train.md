...
!!! warning "deprecated"
    Deprecated command. Prefer **`mmml efield-train`**.
...
```text
usage: mmml ef-train [-h] [--data DATA] [--train-npz TRAIN_NPZ] [--valid-npz VALID_NPZ] [--test-npz TEST_NPZ] [--output-dir OUTPUT_DIR] [--features FEATURES] [--max_degree MAX_DEGREE] [--num_iterations NUM_ITERATIONS] [--num_basis_functions NUM_BASIS_FUNCTIONS]
