...
options:
  ...
  --prelaunch  Serial pre-flight before mpirun: relax strict Tier 2 warnings (spatial env unset, np=1).
  --tier2      Also validate Tier 2 spatial MPI + GPU environment for MLpot.
  --tier3      Survey Tier 3 DOMDEC + MLpot blockers (informational; production blocked).
```
