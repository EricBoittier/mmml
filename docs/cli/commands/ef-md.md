...
!!! warning "deprecated"
    Deprecated command. Prefer **`mmml efield-md`**.
...
```text
usage: mmml ef-md [-h] [--backend {ase,jax}]

Run MD with the electric-field equivariant model (trained via mmml ef-train). Default backend is ASE; use --backend jax for the fully JIT-compiled integrator.
