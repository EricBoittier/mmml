...
```text
usage: mmml active-learning [-h] -i TRAJ [TRAJ ...] [-o OUTPUT] [--max-temp K] [--stride STRIDE] [--max-frames N] [--no-temp-filter]
...
options:
  ...
  -i, --input TRAJ [TRAJ ...]
                        Trajectory file(s) (.traj, .xyz). Globs supported, e.g. 'out/*.traj'
  -o, --output OUTPUT   Output NPZ path (default: md_sampled.npz)
  --max-temp K          Keep only frames with T < max-temp K (default: 300). Ignored if trajectories have no velocities.
