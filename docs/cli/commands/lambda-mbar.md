...
MBAR analysis for a completed lambda-dynamics run. Reads
lambda_ti_snapshots.npz from --run-dir and updates lambda_ti_summary.json.
...
options:
  ...
  --checkpoint CHECKPOINT
                        Override checkpoint (default: read from summary args
                        if present).
  --temperature-K TEMPERATURE_K
                        kT for reduced potentials (default: from summary or
                        100 K).
  --couple-residues COUPLE_RESIDUES
                        Override 1-based coupled residue numbers (default:
                        read from snapshots).
  --ml-cutoff ML_CUTOFF
