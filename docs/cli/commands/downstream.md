...
```text
usage: mmml downstream [-h] --dataset DATASET --checkpoint-dcm CHECKPOINT_DCM --checkpoint-noneq CHECKPOINT_NONEQ [--sample-index SAMPLE_INDEX] [--mode {check,quick,full}] [--output-dir OUTPUT_DIR] [--temperature TEMPERATURE] [--md-steps MD_STEPS] [--timestep TIMESTEP]
                       [--ir-delta IR_DELTA] [--freq-delta FREQ_DELTA] [--opt-fmax OPT_FMAX] [--opt-steps OPT_STEPS] [--raman] [--raman-delta RAMAN_DELTA] [--raman-field RAMAN_FIELD]
...
options:
  ...
  --mode {check,quick,full}
                        Analysis mode: 'check' (single-point), 'quick' (harmonic), 'full' (+MD)
