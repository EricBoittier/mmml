# Menshutkin campaign record (to 2026-08-02) and the general recipe

!!! danger "Archived record — read the status page first"
    This is the campaign page **as it stood on 2026-08-02**, kept verbatim
    because most of it is not repeated anywhere else: the
    [general recipe for reproducing this on another system](#reproducing-this-for-another-system)
    (Steps 0–8 and the checklist), the full
    [bibliography](#literature), the box-building and manifold diagnostics, the
    [bug archaeology](#bugs-found-and-fixed), and the
    [analysis plan](#analysis-plan).

    **Current results and settings live on
    [Menshutkin reaction](menshutkin.md).** Where the two disagree, that page
    wins. Specifically superseded below:

    | claim on this page | what actually happened |
    |---|---|
    | "no PMF from the current checkpoint should be quoted" | Gas 35.09 ± 0.22, water 18.48 ± 0.59, acetonitrile 19.64 ± 0.44 kcal/mol — all reproducing Turan |
    | the solvated runs fail because the model leaves its training manifold | The dominant cause was `stop_gradient` on the ML charges: dropping dq/dR changes the force by 101 % against finite differences |
    | "freeze dq/dR in the force **works**" (`--freeze-charge-forces`) | That *is* the defect. It is now opt-in and defaults **off**; undamped dq/dR is the production setting and is stable |
    | electrostatic embedding needs solvent → solute polarisation to fix the barrier | Polarisation (`ml_mm_pol`) *raises* the barrier and inverts the solvent ordering; dq/dR alone reproduces the reference |
    | the angle wall must come off past the ion pair | Read off the orientational-sampling file rather than the reaction path; on the path a 130° floor never binds, and the wall stays on everywhere |

    Paths below are relative to the repository root, and the host names
    (`gpu09`, `/mmhome/andreychev/...`) are the machines this campaign was run
    on — substitute your own.
...
| File | What it is |
|---|---|
| `_env.sh` | host-aware environment: paths, GPU selection, checkpoint, CV spec |
| `solute.py` | everything reaction-specific -- atom order, CV indices, seeds, model loading |
| `solvent_models.py` | solvent parameters, read from `solvent_params/*.json` |
| `jaxmd_box.py` | CHARMM-free solvated box builder at experimental density |
| `gpu_pairs.py` | static on-device pair list; threads `lambda_t` and `elec_scale` |
| `01_seed_windows.py` | window seeds from the reaction-coordinate scan |
| `02_gas_pmf.sh` | **gas-phase PMF**: seeds, N replicas, merge, MBAR, report |
| `merge_replicas.py` | combines replicas, dropping windows that failed per replica |
| `03_gas_report.py` | profile and figures from the merged run |
| `07_solvated_pmf.py` | solvated PMF: box, walls, coupling ramp, windows, trajectories |
| `08_solvated_mbar.py` | MBAR for the solvated windows |
| `10_extract_solvent_params.py`, `11_extract_all_solvents.sh` | offline CHARMM parameter extraction |
| `run_gas_gpu.sh`, `run_cip_gpu.sh`, `run_train_gpu.sh` | thin GPU launchers |
| `top_mecl.rtf`, `top_chex.rtf` | topology fixes (PDB-legal MECL; CGenFF-typed cyclohexane) |
| `legacy/` | the retired CHARMM-based box path, with a README explaining why |

### Diagnostics (`artifacts/menshutkin/diag/`)

Sixteen active scripts; 27 one-off forensics for solved problems are in
`diag/retired/`.

| Script | Answers |
|---|---|
| `watch_live.py` | how is the running job doing? per-window xi, angle, bond, sub-floor energies |
| `check_structures.py` | are the geometries chemically sensible? bonds, H-C-H, pyramidalisation, attack angle |
| `cv_branch.py` | is the umbrella holding xi while the geometry escapes along the sum? |
| `sum_envelope.py` | what does the training data actually cover? (sets the wall bounds) |
| `term_breakdown.py` | which energy term diverges first, evaluated on the frames before a blow-up |
| `trace_blowup_window.py` | the run-up to a divergence, frame by frame |
| `inspect_blowup.py`, `inspect_internal.py` | contacts and internal geometry at a failure |
| `validate_model.py` | acceptance tests for a checkpoint against known failure modes |
| `tail_charge.py`, `scan_ion_separation.py` | charge and energy versus ion separation |
| `cation_fragment.py`, `dummy_pad.py` | can fragments be described separately? (no -- see below) |
| `manifold_distance.py`, `manifold_control.py`, `solvated_manifold.py` | distance to the training manifold |
| `check_all_solvents.py` | do all five solvent boxes build at the right density? |
| `train_progress.py` | learning rate and validation loss from Orbax checkpoints |
| `make_figures.py` | figures for the README |
...
The figures below are not committed. Regenerate them with
`python artifacts/menshutkin/diag/make_figures.py` after a run; they land next
to the other diagnostics in `artifacts/menshutkin/diag/`.

### Is the model usable?

`model_validity.png` — left: the gas-phase ML energy as Cl is pulled away. It behaves correctly to
...
`training_coverage.png` — training frames per 0.25 Å bin along the reaction
coordinate. `kl.json` has
...
### The gas-phase PMF is done and agrees with the literature

```
barrier         34.56 +/- 0.28 kcal/mol      (Turan et al.: 35.8)
dG reaction    +32.88 kcal/mol               (endothermic, as it must be)
TS location     xi = +0.6 to +1.1 A          (flat within error; see below)
window overlap  0.32                         (healthy)
sampling        5 replicas x 10 ps x 30 windows, 3600 frames/window
                every replica contributed to every window; none dropped
```

| xi (A) | PMF | | xi (A) | PMF |
|---|---|---|---|---|
| -1.30 | 0.00 | | +0.60 | 34.06 |
| -0.80 | 4.13 | | +0.80 | 34.13 |
| -0.30 | 14.12 | | **+1.10** | **34.56** |
| 0.00 | 22.48 | | +1.40 | 33.17 |
| +0.30 | 30.34 | | +1.60 | 32.91 |

Two things to read correctly. The reaction is strongly **endothermic** in the gas
phase (+32.9 kcal/mol) -- the ion pair sits far above the neutrals, which the
reference dipoles independently confirm (see "Can the two ions be described
separately past the cutoff?"). And the barrier is **late**, as Hammond's
postulate requires for an endothermic reaction. Do not quote a TS at xi = +1.10:
the profile reads 34.06, 34.05, 34.13, 34.33, 34.56, 34.56 across xi = +0.6 to
+1.1, which is flat inside the +-0.25 error bars.

Reproduce with `bash examples/menshutkin/02_gas_pmf.sh` (about 30 min on one
GPU). `SMOKE=1` runs a 2 x 0.5 ps version in a few minutes.

### What works

| Piece | State |
|---|---|
| Environment, GPU, timestep units | done |
| Shared CV / walls / `mm_bonded` / `rxncoor` / `ml_mm_elec` + tests | done |
| Solvent boxes, all five solvents at experimental density | done |
| **Gas-phase PMF** | **done, validated against Turan** |
| Solvated, mechanical embedding | runs; the A/B baseline |
| Solvated, electrostatic embedding | runs only with `--freeze-charge-forces` |
| Analysis suite | not started |
| Model retrain at long cutoff | started, stopped at epoch 1436/5000 |

### The one thing that governs everything: xi does not determine a geometry

`xi = r(C-Cl) - r(C-N)` fixes one number. The umbrella bias constrains that
number and nothing else, and three unconstrained directions turned out to matter.
Each produced sampling that looked *fine* -- finite energies, smooth histograms,
no crash -- while being wrong.

| Free direction | What the sampling did | Restraint |
|---|---|---|
| the sum `r(C-Cl) + r(C-N)` | methyl drifted from **both** partners; sum 6.25 A where training never exceeds 4.44 A at that xi | `BondRetentionWall`, `min(r) <= 2.25 A` |
| the attack angle `N-C-Cl` | chloride swung round to hydrogen-bond with the ammonium; mean **70 deg** past xi = +1.3, where the reaction channel is 165-173 deg | `AngleWall`, `angle >= 130 deg` |
| (with REX on) which window a configuration belongs to | a reset window's configuration propagated along the ladder; 24 of 30 windows corrupted | recovery + replica exchange now refuses to run |

The bounds are taken from the training data, not chosen: `min(r)` never exceeds
2.18 A anywhere in the training set at any xi, and the reaction channel sits at
165-173 deg. `artifacts/menshutkin/diag/sum_envelope.py` and `cv_branch.py`
measure them.

**The dangerous case is the silent one.** Windows sampling at 70 deg did not
crash -- they ran to completion producing perfectly finite numbers that would
have gone into MBAR and produced a smooth, entirely meaningless profile. The
crashes were the lucky failures.

### Electrostatic embedding: a feedback loop, and what it costs to break it

Solvated runs with `ml_mm_elec` at full strength diverge within ~90 fs. The
mechanism is a loop: the solvent pulls the chloride out, the model responds with
more charge transfer, the larger charge pulls the solvent in harder. Measured
directly (`diag/term_breakdown.py`): q(Cl) ran -0.80 -> -1.035 over 50 fs while
the solute-solvent attraction grew with it and the solute's own ML energy climbed
4.6 eV.
...
| Attempt | Result |
|---|---|
| erf damping on the 1/r (sigma = 1 A) | fixed a *different* collapse (water H at 1.586 A from Cl); not this one |
| ramp the coupling on in 5 / 10 / 20 stages | failed at 0.80 / 0.70 / **0.60** -- finer is *worse*, so a threshold, not a rate |
| minimise with the coupling off | necessary (it otherwise drove xi from 0.00 to +0.94 at 0 K) but not sufficient |
| clip \|q\| at 1 e | no effect: the runaway is already underway at \|q\| ~ 0.85, inside the physical range |
| **freeze dq/dR in the force** | **stable through the full ramp to 1.00 and all windows complete** |

So the loop gain is the charge-response force specifically, not the Coulomb
magnitude. `--freeze-charge-forces` recomputes the charges every step and keeps
them in the energy, but stops the gradient through them.

**This is an approximation and must be reported as one.** The forces are then not
the exact gradient of the energy, so umbrella sampling plus MBAR -- which assumes
samples come from the Boltzmann distribution of the biased Hamiltonian -- holds
only approximately. It is not comparable to the `stop_gradient` on the MIC
lattice shift in `devtools/CLAUDE.md`, which is exact because piecewise-constant
shifts genuinely have zero derivative.

Validation plan, cheapest first:

1. compare the frozen-force PMF against mechanical embedding -- it should lie
   between mechanical and the true answer;
2. check the barrier lowering versus gas phase against Turan's solvated numbers;
3. a short fully-variational run at reduced coupling, to bound the free-energy
   difference.
...
The gas-phase PMF now reproduces the literature, and the reason earlier attempts
did not was sampling, not the model.

- **Gas-phase PMF done: 34.56 +- 0.28 kcal/mol** against Turan's 35.8, with
  dG_reaction = +32.9 (correctly endothermic). 5 independent replicas x 10 ps x
  30 windows.
- **Three unconstrained directions found and walled.** `xi` fixes one number;
...
- **Replica exchange + per-window recovery is now refused.** Combining them
  corrupted 24 of 30 windows silently: a reset window's configuration
  propagates along the ladder. The gas driver no longer uses REX, which also
  makes the windows genuinely independent -- so a failed window costs one
  replica, not the campaign.
- **Multi-replica strategy.** This checkpoint sustains ~8 ps per window before
  some window finds a spurious well (0.5 ps and 8 ps runs clean; a 55 ps run
  left three windows resetting 106, 84 and 42 times). Five short replicas give
  the same sampling, are uncorrelated, and lose only one replica when a window
  fails. `merge_replicas.py` drops failed windows per replica and reports
  coverage.
- **Electrostatic embedding: mechanism identified.** The instability is a
  charge-response feedback loop whose gain exceeds unity near 0.6 of full
  coupling. Ramping (5/10/20 stages -> failure at 0.80/0.70/0.60) and clipping
  |q| both fail; freezing dq/dR in the force works. Available as
  `--freeze-charge-forces`, documented as an approximation.
- **Live trajectory output.** `umbrella_live.npz` is flushed during the run and
  `diag/watch_live.py` reports per-window xi tracking, attack angle, shortest
  bond and sub-floor energies while the job is still going.
- **`PYTHONUNBUFFERED=1` in the gas driver.** Without it the log lagged
  thousands of steps behind the run; several wrong conclusions about when runs
  died came from reading stale buffers.
- Diagnostics audited: 42 scripts down to 16 active, 27 moved to
  `diag/retired/`. Corrupted output quarantined in
  `artifacts/menshutkin/gas/CORRUPTED_rex_recovery/` with a README explaining
