...
| | frames | share |
|---|---:|---:|
| **Typeable — usable for hybrid ML/MM** | **6,103** | **32.9%** |
| Dropped: no CGenFF template for the composition | 5,718 | 30.8% |
| Dropped: composition matched, topology did not | 5,198 | 28.0% |
| Dropped: not two covalent components | 946 | 5.1% |
| Dropped: not graph-isomorphic to the chosen template | 583 | 3.1% |

**83 of 1,333 monomer species** are typeable, and they form **919 distinct
...
- **No template (30.8%)** — the composition has no CGenFF `RESI` at all. This is
  the noble gases, the bare ions, and small molecules CGenFF simply does not
  cover (H₂S, CH₂O, H₂S₂ are the three biggest single contributors).
- **Topology mismatch (28.0%)** — a residue with the *same formula* exists, but
...
Largest untypeable monomers by occurrence:

| Monomer | occurrences | Monomer | occurrences |
|---|---:|---|---:|
| H₂S | 13,256 | He | 4,199 |
| CH₂O | 13,040 | Ne | 3,793 |
| Ar | 7,154 | C₂H₄O | 3,682 |
| H₂S₂ | 7,147 | Kr | 3,533 |
| C₆H₁₂ | 6,971 | F⁻ | 3,533 |
| Cl⁻ | 6,808 | Xe | 3,452 |
...
<figcaption>90 of 164 CGenFF nonbonded types are reachable from the DES dimer set, via 83 residues.</figcaption>
</figure>
...
if some training frame contains an atom of that type. **90 of 164** types
qualify; the other 74 are inert no matter how long you train.

The distribution is steep. `HGA3` (aliphatic H) appears in 81% of typeable
frames and `CG331` (methyl C) in 77%, while 25 of the 90 types appear in fewer
...
[σ/ε degeneracy](hybrid-mm-lj-scales.md) bites. Freeze them or exclude them
rather than trusting a fitted value.

### Most-exercised types

| CGenFF type | σ (Å) | ε (kcal/mol) | sampled frames | residues |
|---|---:|---:|---:|---|
| `HGA3` | 2.3876 | 0.02400 | 4,918 | 54 |
| `CG331` | 3.6527 | 0.07800 | 4,677 | 51 |
| `HT` | 0.4000 | 0.04600 | 3,398 | TIP3 only |
| `OT` | 3.1506 | 0.15210 | 3,398 | TIP3 only |
| `HGP1` | 0.4000 | 0.04600 | 2,518 | 21 |
| `HGA2` | 2.3876 | 0.03500 | 2,210 | 32 |
| `CG321` | 3.5814 | 0.05600 | 1,862 | 28 |
| `OG311` | 3.1449 | 0.19210 | 1,393 | 9 |
| `HGR52` | 1.6036 | 0.04600 | 1,356 | 12 |
| `OG2D1` | 3.0291 | 0.12000 | 1,258 | 10 |
...
**Full tables** (generated, 90 types and 83 residues):
...
```
TIP3 METH AMM1 ETHE MEOH FORH ETOH ACEH IMIA ETSH ACO  ACEM MAM1 FORM
MESH BENZ ETHA BUTA PHEN DMDS NH4  FORA PRPA DMAM PYRL PYR1 MAS  PRO2
EMS  TMAM DETE ACET MIMI PRLD CPEN MGUA THF  MAMM IMIM BTE1 PRAM ETAC
PENT SM073 DMA HEXA THPS DITH DEDS INDO TOLU NC4  TRIT EAMM GUAN FETH
FLUB NC3  ACN  CLET DCM  PRPY DCLE DFET BRET DBRE EIMI MIND EBEN EIND
PROA EIMM EPHE DMEP TFET SM158 MP_0 GLYN SM129 TCLE AANM DME  DFB
```
...
## Wiring it into the `lj_scales` ladder

The [`examples/lj_scales`](https://github.com/EricBoittier/mmml/tree/main/examples/lj_scales)
ladder consumes a **dense padded NPZ** (`R (n,atoms,3)`, `Z`, `N`, `E`, `F`)
that `prepare-mm-dataset` then enriches with `cgenff_type_idx` /
`cgenff_charge` / `mol_id`. The DES set is HDF5, so the bridge is a conversion
step that does not exist yet:

```
qcell_dimers.h5  ──[missing: h5 → padded NPZ]──▶  LJ_DATASET
                                                     │
                                                     ▼
                                          ...
                                          (prepare-mm-dataset)
                                                     │
                                                     ▼
                                          05_train.sh  learn_mm_lj_scales
```

`mmml/models/physnetjax/physnetjax/data/read_h5.py` already reads this exact
layout into padded arrays (`prepare_h5_datasets`), so the converter is a thin
wrapper over it rather than new parsing work. Pad width must be **34** (panel
(d)), not the ladder's default `LJ_PAD_TO=20`.

Three things to settle before running it:

1. **Units.** The ladder's existing datasets are in eV / eV·Å; `qcell_dimers.h5`
   stores `formation_energy` in eV referenced to `metadata/free_atom_energy`.
   Confirm before mixing — see the units footgun in
   [`docs/UNITS_SUMMARY.md`](UNITS_SUMMARY.md).
2. **Yield.** 32.9% of frames survive assignment, so ~122k of the 371k frames
   are usable. That is still ample for `LJ_NTRAIN=8000`, but the surviving set
   is water-heavy and its composition is *not* the composition of the raw file.
