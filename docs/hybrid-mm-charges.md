...
change them when enabling Mode C on the MD side.
...
- **Train:** `--hybrid-mm` without `--mm-charge-correction`
...
## Mode B — `latent` (**deferred**)

```text
q_MM = neutralize_per_monomer(q_ML)
```

Replace CGenFF in `E_MM`; do not add.  **Not implemented** in hybrid train or
the MD calculator.  Named so nobody invents a third CLI flag by accident.
Closest cousin: fluctuating embedding in `examples/cg_jaxmd.py` (different
product).

Calling `apply_mm_charge_mode("latent", ...)` raises `NotImplementedError`.

---

## Mode C — `fixed_plus_latent`

```text
q_MM = q_CGenFF + neutralize_per_monomer(q_ML)
```

- **Train:** `--mm-charge-correction` (requires `--charges`)
...
- **MD v1:** dimers only (`n_monomers == 2`), AB-context charges, frozen-`q`
  MM forces (matches training).  Liquids and JAX-PME refuse Mode C until a
...
CLI: train `--mm-charge-correction`; MD / parity `--mm-charge-correction`
(aliased to Mode C).  Checkpoint sidecar `hybrid_mm.json` records the mode;
MD warns on mismatch but still requires explicit opt-in.

---

## Liquid follow-up (blocked until dimer Mode C parity is green)
...
| Option | Idea |
|--------|------|
| **L1** | Per-monomer `q_ML` (environment-free) + Mode C — simple, systematically unlike train AB context |
