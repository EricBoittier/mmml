...
def plot_3d(grid, esp, atoms=None):
    """
    Create 3D visualization of ESP on grid with molecular structure.
    
    Creates a 6-panel figure showing ESP from different viewing angles
    along with molecular structure representations.
    ...
    esp : array_like
        ESP values at grid points, shape (N,)
    atoms : ase.Atoms, optional
        ASE Atoms object for molecular visualization, by default None
    """
    scat_size = 10
...
def read_xyz(testdata: Path):
    ...
    ----------
    testdata : Path
        Path to directory containing xyz file
        
    Returns
    -------
    tuple
        (coordinates, atom_types) where:
        - coordinates: Atomic coordinates, shape (N, 3)
        - atom_types: Element symbols, shape (N,)
    """
    # read xyz file
...
def load_data(
    ...
    """
    Load multipole data from files.
    
    Loads multipole data from NPZ file and ESP/grid data from text files,
    with optional distance-based culling of grid points.
    ...
    cull : bool, optional
        Whether to cull grid points by distance, by default False
    cull_min : float, optional
        Minimum distance from atoms for grid points, by default 3.0
    cull_max : float, optional
        Maximum distance from atoms for grid points, by default 4.5
        
    Returns
    -------
    dict
        Dictionary containing multipole data and grid information
    """
    # print(testdata)
    # read npz file
...
def calc_mono_esp(data):
    """
    Calculate ESP from monopoles only.
    
    Parameters
    ----------
    data : dict
        Dictionary containing 'monopoles', 'xyz', and 'grid' keys
        
    Returns
    -------
    array_like
        ESP values from monopoles only
    """
...
def calc_dipole_esp(data):
    """
    Calculate ESP from dipoles only.
    
    Parameters
    ----------
    data : dict
        Dictionary containing 'dipoles', 'xyz', and 'grid' keys
        
    Returns
    -------
    array_like
        ESP values from dipoles only
    """
...
def calc_quad_esp(data):
    """
    Calculate ESP from quadrupoles only.
    
    Parameters
    ----------
    data : dict
        Dictionary containing 'quadrupoles', 'xyz', and 'grid' keys
        
    Returns
    -------
    array_like
        ESP values from quadrupoles only
    """
    calc_esp = np.zeros(data["esp"].shape)
...
def plot_esp(esp, calc_esp, title=""):
    """
    Plot predicted vs reference ESP values.
    
    Creates a scatter plot comparing predicted and reference ESP values
    with RMSE calculation and diagonal reference line.
...
def calc_esp_from_multipoles(data):
    """
    Calculate ESP from monopoles, dipoles, and quadrupoles.
    ...
    -------
    tuple
        (monopole_esp, dipole_esp, quad_esp) - ESP contributions from each multipole type
    """
...
def plot_moments(data):
    """
    Plot multipole moments as heatmaps.
    
    Creates a 3-panel figure showing monopoles, dipoles, and quadrupoles
    as heatmaps for visual comparison.
...
def plot_esp_slices(data):
    ...
    Creates a 3-panel figure showing ESP slices in different planes
    for 3D visualization.
    
    Parameters
    ----------
    data : dict
        Dictionary containing 'grid' and 'esp' keys
    """
    fig, axs = plt.subplots(1, 3, figsize=(5, 5), sharey=True, sharex=True)
