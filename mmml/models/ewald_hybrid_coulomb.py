...
def ewald_static_params_from_box_length(
    box_length_A: float,
    *,
    accuracy: float = 1e-6,
    real_space_cutoff_A: float | None = None,
) -> tuple[float, np.ndarray]:
    """Host-side ``(alpha, n_int)`` for a cubic box (static k-grid factory inputs)."""
    L = float(box_length_A)
    if not np.isfinite(L) or L <= 0.0:
        raise ValueError(f"box_length_A must be positive and finite, got {box_length_A!r}")
    accuracy_exponent = math.sqrt(max(-math.log(float(accuracy)), 1.0))
    rcut_for_alpha = (
        float(real_space_cutoff_A) if real_space_cutoff_A is not None else L / 2.0
    )
    alpha = default_ewald_alpha(rcut_for_alpha, accuracy_exponent=accuracy_exponent)
    cell_np = np.diag([L, L, L])
    n_int = build_kspace_integers(cell_np, alpha, accuracy_exponent=accuracy_exponent)
    return float(alpha), np.asarray(n_int, dtype=np.int32)


def hybrid_ewald_coulomb_energy_with_cell(
    positions: Array,
    mol_id: Array,
    charges: Array,
    cell: Array,
    *,
    ...
    """Full-box or cross-monomer Ewald with a traced ``cell`` (NpT-friendly).

    ``alpha`` and ``n_int`` are host/static factory outputs from
    :func:`ewald_static_params_from_box_length`. Cartesian reciprocal vectors
    ``k = n_int @ B(cell)`` update with the live cell each call.
    """
    ...
    if cell.shape == (3,):
        cell = jnp.diag(cell)
    elif cell.ndim == 0 or cell.size == 1:
        side = jnp.reshape(cell, ())
        cell = jnp.eye(3, dtype=pos.dtype) * side
    valid = mid >= 0
    q_full = jnp.where(valid, q, 0.0)
    alpha_f = float(alpha)
    n_int_arr = jnp.asarray(n_int)
    ...
    real_mat = qq * jax.scipy.special.erfc(alpha_f * dij) / dij
    ...
    if include_intramolecular:
        e_recip = ewald_reciprocal_energy(pos, q_full, cell, n_int_arr, alpha_f)
    else:
        volume = jnp.abs(jnp.linalg.det(cell))
        recip = 2.0 * jnp.pi * jnp.linalg.inv(cell).T
        k = jnp.asarray(n_int_arr, dtype=cell.dtype) @ recip
...
def hybrid_ewald_coulomb_energy(
    ...
    *,
    ...
    Python values at the call site that builds ``alpha``/``n_int``. For NpT MD
    with a live ``box_override``, use :func:`hybrid_ewald_coulomb_energy_with_cell`
    instead (see ``mm_energy_forces``).
    """
    L = float(box_length_A)
    ...
    cell = jnp.asarray(np.diag([L, L, L]))
    return hybrid_ewald_coulomb_energy_with_cell(
        ...
        include_intramolecular=include_intramolecular,
    )
