...
if cuda:
    print(
        f"Running CHARMM/BLaDE MD example on computer with {num_devices} CUDA devices".format(
            num_devices
        )
    )
    print(
        f"Running on device {torch.cuda.current_device()} which is a {torch.cuda.get_device_name(torch.cuda.current_device())}"
    )
    import sys
...
def run_md(
    ...
    DYNA STaRT CPT LEAP NSTE 10000 TIME 0.0005 -
   INBFRQ -1 IMGFRQ 50 IHBFRQ 0 -
   IUNREA -1 IUNWRI 31 IUNCRD 32 IUNVEL -1 -
   IHTFRQ 0 IEQFRQ 1000 NTRFRQ 1000 -
   NPRINT 100 IPRFRQ 1000 NSAVC 1000  -
   PCONSTANT PREF   1.0  PMASS 100  PGAMMA   20.0 -
