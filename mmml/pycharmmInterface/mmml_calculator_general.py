...
from mmml.pycharmmInterface.cell_list import (
...
def indices_of_pairs(a, b, n_atoms=5, n_mol=20, monomer_offsets=None, atoms_per_monomer_list=None):
...
def indices_of_monomer(a, n_atoms=5, n_mol=20, monomer_offsets=None, atoms_per_monomer_list=None):
...
def setup_calculator(
    ...
    if cell:
        ...
        mol_id = None
        try:
            # Build mol_id supporting variable monomer sizes:
            # e.g. [0, 0, 0, 1, 1, 1, 2, 2, 2, 2, 2, 2] for atoms_per_monomer=[3,3,6]
            mol_id_parts = [
                i * jnp.ones(atoms_per_monomer_list[i], dtype=jnp.int32)
    ...
    def get_MM_energy_forces_fns(
        ...
        and ``monomer_offsets``.
        """
        # Loading from pycharmm (for consistency), will consider moving 
        ...
        # --- Build inter-monomer pair indices (heterogeneous sizes) ----------
        # Strategy: when PBC is enabled, use a cell list for O(N)-scaling
        # pair generation; otherwise fall back to the Cartesian-product approach.
        _dp = dimer_permutations(N_MONOMERS)
        pbc_cell = cell if do_pbc_map else None

        _use_cell_list = (pbc_cell is not None)
        ...
        if _use_cell_list:
            ...
            pair_idx_atom_atom = jnp.stack([_cl_i, _cl_j], axis=1)  # (max_pairs, 2)
            ...
            # Build per-pair lambda from monomer membership
            ...
            # For the switching function we need per-dimer pair counts.
            # With the cell list, pairs are not grouped by dimer, so we
            # build a per-pair dimer index and count per dimer.
            _dimer_lookup = {}
            ...
            _cl_mask_jnp = None  # no masking needed for all-pairs
            pair_dimer_idx = None
        ...
        masses = np.array(psf.get_amass())[:total_atoms]
        ...
        atomtype_codes = np.array(psf.get_atype())[:total_atoms]
        ...
        # Per-dimer pair counts for the switching function expansion
        ...
        def get_switching_function(
            ...
            def apply_switching_function(positions: Array, pair_energies: Array) -> Array:
                # Compute per-monomer COMs supporting variable sizes
                coms = jnp.stack([
                ...
                if pair_dimer_idx is not None:
                    # Cell-list path: each pair has its own dimer index.
                    # Pairs with dimer_idx == -1 (invalid) get scale 0.
                    ...
                    # All-pairs fallback: pairs are grouped by dimer
                    mm_scale_expanded = jnp.concatenate([
                        jnp.full((n_pairs_per_dimer_arr[d],), mm_scale[d])
        ...
        def calculate_mm_energy(positions: Array) -> Array:
            ...
            # Cell-list mask (already baked into pair_ep/pair_qq via pair_lambda_mm,
            # but we also need to suppress LJ/Coulomb for padding slots where
            # distance == 0 would cause numerical issues).
            ...
            electrostatic_energies = coulomb(distances, pair_qq)
            return (vdw_energies + electrostatic_energies).sum()
