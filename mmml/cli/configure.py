...
def _report_written(path: Path, *, next_hint: str | None = None) -> None:
    report = get_reporter()
    report.status("success", "Wrote configuration", detail=str(path))
    if next_hint:
        report.status("info", "Next", detail=next_hint)


def _report_written_files(
    root: Path,
    paths: list[Path],
    *,
    title: str,
    ...
    rows = [(str(path.relative_to(root)),) for path in shown]
    if limit is not None and len(paths) > limit:
        rows.append((f"… and {len(paths) - limit} more files",))
    report = get_reporter()
    report.status("success", title, detail=str(root))
    report.table("Files", ("Path",), rows)
    if next_hint:
        report.status("info", "Next", detail=next_hint)
...
def wizard_md_campaign(out_dir: Path) -> list[Path]:
    ...
    get_reporter().status(
        "info", "Single job", detail=f"mmml md-system --config {path} --job-id <name>"
    )
    return [path]
...
def wizard_snakemake_md(out_dir: Path) -> list[Path]:
    ...
    _report_written_files(
        out_dir,
        written,
        title="Wrote workflow",
        next_hint=f"cd {wf_name} && snakemake -n",
    )
    return written
...
def configure_main(argv: list[str] | None = None) -> int:
    ...
    if args.preset:
        ...
        _report_written_files(
            out_dir,
            paths,
            title=f"Applied preset {args.preset!r}",
            next_hint=preset.run_hint,
            limit=12,
        )
        return 0
...
def _configure_interactive(args: argparse.Namespace, out_dir: Path) -> int:
    ...
    if workflow == "preset-menu":
        ...
        _report_written_files(
            out_dir,
            paths,
            title=f"Wrote preset {preset_key!r}",
            next_hint=preset.run_hint,
            limit=8,
        )
        return 0
