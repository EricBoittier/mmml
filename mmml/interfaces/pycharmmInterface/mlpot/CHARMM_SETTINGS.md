...
**Staged heating:** `--n-heat-segments N` splits `--ps-heat` into short chained restarts (`heat_{tag}.0.res`, …) with overlap rescue between segments. Default DCM:9 script uses `N_HEAT_SEGMENTS=4` (5 ps per segment for 20 ps total). Each segment gets a linear slice of `--heat-firstt`→`--heat-finalt`. Overlap/extent guards default to mid-segment checks every `--dynamics-overlap-check-interval` steps (not only at segment end). Use `--heat-overlap-segment-boundary-only` to restore legacy end-of-segment-only checks (Hoover CPT handoff). Before each heat `dyna`, `finalize_heat_dynamics_frequencies()` harmonizes `ihtfrq` / `iprfrq` with `nstep` and recomputes `TEMINC`.

**ML/MM cutoffs (code defaults):** `--mm-switch-on 8`, `--mm-switch-width 5`, `--ml-switch-width 1.5` (`extended_mm5` from DCM:3 NVE sweep). `run_dcm9_stability.sh` passes the same via `MM_SWITCH_ON` / `MM_SWITCH_WIDTH` / `ML_SWITCH_WIDTH`. Plots: [`docs/mlpot-settings.md`](../../../../docs/mlpot-settings.md) (regenerate with `uv run python scripts/plot_mlpot_settings.py`).
...
Neighbor lists for large PBC systems use an outer cutoff **`mm_switch_on + mm_switch_width` (13 Å with defaults 8 + 5)** plus optional jax-md skin — pairs near that cutoff stay listed so they can enter range later. That is separate from `mm_r_min`.
