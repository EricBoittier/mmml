...
| **heat** | Verlet + velocity scaling | Verlet + IMAGE lists | `firstt=0.2×T`, `finalt=T`, `TEMINC` | **`--heat-ihtfrq` or `--dyn-nprint`**; default **`--heat-comp-damp`** (COMP force-damp on high-\|F\| **H** only, `iasvel=0`; cleared before equi/NVE/prod) |
