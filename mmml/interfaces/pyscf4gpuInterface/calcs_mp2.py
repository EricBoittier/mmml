...
def compute_mp2_single(
    ...
    efield = (
        None
        if efield is None
        else np.asarray(efield, dtype=np.float64).reshape(3)
    )
    ...
    if efield is not None:
        ...
        mf, e_hf = run_scf_uniform_efield(
            efield,
            mol,
            xc=None,
            include_nuclear_field_energy=efield_include_nuclear_energy,
    ...
    if energy:
        out["energy"] = np.array(e_mp2)
        out["energy_hf"] = np.array(e_hf)
    ...
    if dipole:
        dm = mf.make_rdm1()
        out["D"] = np.asarray(
            mf.dip_moment(unit="DEBYE", dm=dm, verbose=0)
        )
...
def compute_mp2_batch(
    ...
    """
    Run DF-MP2 for multiple geometries in one process/GPU context.

    Parameters
    ----------
    R_batch
        (ngeom, natom, 3) Cartesian coordinates in Angstrom.

    efield
        Either:
            (3,)      same field for all geometries
            (ngeom,3) different field per geometry
    ...
    -------
    dict
        E          MP2 total energies
        E_hf       HF energies
        E_corr     MP2 correlation energies
        F          Forces (-gradient), if requested
        Dxyz       Dipoles, if requested
        Ef         Electric fields, if supplied
        R, Z, N
    """
    n = R_batch.shape[0]
    Z = np.asarray(Z)
    ...
    if efield is not None:
        ...
        if efield_arr.shape == (3,):
            ...
            raise ValueError(
                f"efield must be (3,) or (n,3) with n={n}, "
                f"got {efield_arr.shape}"
            )
    ...
    # Main loop
    #
    for i in tqdm(range(n), desc="pyscf-mp2", unit="geom"):

        ef_i = None if efield_arr is None else efield_arr[i]

        out = compute_mp2_single(
            R_batch[i],
            Z,
            basis=basis,
            spin=spin,
        ...
        if energy:
            energies.append(out["energy"])
            hf_energies.append(out["energy_hf"])
            corr_energies.append(out["energy_corr"])
    ...
    if energy:
        result["E"] = np.stack(energies)
        result["E_hf"] = np.stack(hf_energies)
        result["E_corr"] = np.stack(corr_energies)

    if gradient:
        result["F"] = -np.stack(gradients)
