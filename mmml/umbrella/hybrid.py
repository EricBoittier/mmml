...
def _build_window_leg(
    ...
    *,
    ...
    # Fallback matches the dataclass default, so a config built elsewhere
    # cannot silently take the other pair-list path.
...
def _relax_and_gate_seed(win_system, energy, seeded, *, box, ml_indices, cfg):
    """FIRE-relax the surroundings of the frozen seed, then read its forces.

    Returns ``(win_system, seeded, n_relax, fmax_ml, fmax_all, seed_error)``.
    Both steps go through the ASE face, which some ``lr_solver`` choices do not
    have; there the relaxation and the gate are skipped rather than failing.
    """
    ...
    try:
        ...
        # No usable ASE face here: the relaxation and the gate are both
        # skipped, as before.
        fmax = float("nan")
        fmax_all = float("nan")
    except Exception as exc:  # noqa: BLE001
        # A blown-up minimisation must cost one window, not the campaign: the
        # serial path runs all 30 in a single process.
        seed_error = f"seed relaxation failed: {type(exc).__name__}: {exc}"
    return win_system, seeded, n_relax, fmax, fmax_all, seed_error
...
def run_umbrella_hybrid_nvt(cfg: UmbrellaConfig) -> UmbrellaResult:
    ...
    # Carry the previous window's final frame forward, so the relaxed solvation
    # shell travels along the ladder instead of every window re-forming it from
    # the same packed configuration.
    #
    # Only when the whole ladder runs in order. Under --resume or --windows the
    # set of windows that happen to be missing decides what "previous" means, so
    # a chained window would sample a different ensemble on a resumed run than
    # on a fresh one -- a silent, unreproducible difference in the PMF.
    chain = bool(getattr(cfg, "seed_from_previous_window", False))
    if chain and to_run != list(range(k_windows)):
        chain = False
        print(
            "  seed_from_previous_window disabled: chaining is only defined for "
            "the full ladder in order, and this run covers "
            f"{len(to_run)}/{k_windows} window(s)",
            flush=True,
        )
    ...
    for wid in to_run:
        ...
        win_system, energy, neighbor_fn = _build_leg(
            seeded, xi0, k_w, verbose_pairs=(wid == to_run[0])
        )
        win_system, seeded, n_relax, fmax, fmax_all, seed_error = _relax_and_gate_seed(
            win_system, energy, seeded, box=box, ml_indices=ml_indices, cfg=cfg
        )
                ...
                if frames is None:
                    ...
                    if not (
                        ...
                        if not np.all(np.isfinite(cv_w)):
                            ...
                            if chain:
                                # Hand the relaxed solvent to the next window.
                                # Only reached on the success path, so a failed
                                # or diverged window leaves the previous seed in
                                # place instead of poisoning every window after
                                # it with a NaN that still looks finite in the
                                # aggregate.
                                seed_source = np.asarray(
                                    pos_w[-1], dtype=np.float64
                                )
