...
class EuclideanFastAttention(nn.Module):
    ...
    @nn.compact
    def __call__(
        self, inputs: Array, positions: Array, batch_segments: Array, graph_mask: Array
    ):
        """
        ...
        num_features_qk = (
            inputs.shape[-1] if self.num_features_qk is None else self.num_features_qk
        )

        num_features_v = (
            inputs.shape[-1] if self.num_features_v is None else self.num_features_v
        )

        if self.parametrized:
            q = e3x.nn.Dense(num_features_qk, dtype=self.param_dtype)(
                inputs
            )  # (N, 1 or 2, (max_degree_in + 1)**2, qk_num_features)
            k = e3x.nn.Dense(num_features_qk, dtype=self.param_dtype)(
                inputs
            )  # (N, 1 or 2, (max_degree_in + 1)**2, qk_num_features)
            v = e3x.nn.Dense(num_features_v, dtype=self.param_dtype)(
                inputs
            )  # (N, 1 or 2, (max_degree_in + 1)**2, qk_num_features)
        else:
        ...
        with jax.ensure_compile_time_eval():
            grid_u, grid_w = e3x.so3.lebedev_quadrature(num=self.lebedev_num)
        ...
        # the spherical harmonics expansion for all grid points and then sums over the grid points.
            ...
            grid_u_sph = jnp.expand_dims(
                e3x.so3.spherical_harmonics(grid_u, max_degree=ti_max_degree_sph),
                axis=(-3, -1),
            )  # (M, 1, (L_Y+1)**2), 1)
            ...
            if self.ti_degree_scaling_constants is not None:
                ...
                c = jnp.repeat(
                    c, repeats=repeats, total_repeat_length=repeats.sum()
                )  # ((L_Y+1)**2, )
            ...
            return tensor_integration.TensorIntegration(
                ...
                jnp.repeat(grid_u_sph, axis=-1, repeats=beta.shape[-1]), beta, grid_w
            )  # (N, M, P, (L_out+1)**2, F)
