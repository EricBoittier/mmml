...
!     IQLDM_SOFTCORE whether to use Hayes et al JPC B (2017) soft cores - domdec only
  ...
  integer, allocatable, dimension(:) :: iblcks_iat, iblcks_ires ! atom and restraint indices
  real(chm_real),save :: kscalecons
  integer,save :: nires
  ! soft bond setup
  logical,save :: qsobo
  integer,save :: nsobo
...
! Multi-Site lambda-dynamics using QTHETA dynamics
  ...
  logical, save :: qreadlambdafromrestart = .true.
...
!> Turns msld soft cores with domdec on or off. (Not implemented outside domdec)
   subroutine ldm_scatmsld_setup(comlyn,comlen)
      ...
      if (INDXA(COMLYN,COMLEN,'ON').gt.0) then
         if( prnlev >= 2 ) then
            ...
            write(outu,'(a)') 'Constraints must be set up with "scat k [value]"'
            write(outu,'(a)') 'Warning, only works with domdec'
            write(outu,'(a)') ''
         endif
         if (.not. qldm_scalecons) then
            call chmalloc('lambda_ltm.src','ldm_scatmsld_setup','iblcks',maxaim,intg=iblcks)
            call chmalloc('lambda_ltm.src','ldm_scatmsld_setup','iblcks_iat',maxaim,intg=iblcks_iat)
            call chmalloc('lambda_ltm.src','ldm_scatmsld_setup','iblcks_ires',maxaim,intg=iblcks_ires)
         endif
         iblcks = 1
         iblcks_iat = 0
         iblcks_ires = 0
         ...
         nires = 0
         qldm_scalecons = .true.
      else if (INDXA(COMLYN,COMLEN,'K').gt.0) then
         if (.not. qldm_scalecons) then
            CALL WRNDIE(-5,'<BLOCK>', &
                 'Failed attempt to set scat restraint constant. Call "SCAT ON" before "SCAT K [VALUE]".')
         endif
         if( prnlev >= 2 ) then
            write(outu,'(a)') 'Setting k constant for MSLD scaling of'
            write(outu,'(a)') 'constrained atoms'
            write(outu,'(a)') ''
         endif
         kscalecons = nextf(COMLYN,COMLEN)
      else
         if( prnlev >= 2 ) then
         ...
         if (qldm_scalecons) then
            ...
            call chmdealloc('lambda_ltm.src','ldm_scatmsld_setup','iblcks_iat',maxaim,intg=iblcks_iat)
            call chmdealloc('lambda_ltm.src','ldm_scatmsld_setup','iblcks_ires',maxaim,intg=iblcks_ires)
         endif
         nires = 0
         qldm_scalecons = .false.
      endif
  ...
  ! use block_fcm, only : blasgn ! Circular dependency, use local msld_blasgn copy
    ...
    nires = nires + 1
    iblcks_ires(nires+1) = iblcks_ires(nires)
    DO I=1, NATOM
       IF (ITEMP(I)  ==  1) THEN
          iblcks(i) = iblckp(i)
          iblcks_ires(nires+1) = iblcks_ires(nires+1) + 1
          iblcks_iat(iblcks_ires(nires+1)) = i
       ENDIF
    ENDDO
...
30     FORMAT(' The selected pair of atoms have been designated as a soft bond', I4, I4)
    ...
    !---------------------------------------------------------------------------
  subroutine ldm_readlambdafromrestart(comlyn,comlen)
    !         invoked by BLOCK subcommand "RLFR"
    !---------------------------------------------------------------------------
    use stream
    use string
    ...
    if (INDXA(COMLYN,COMLEN,'ON').gt.0) then
       if( prnlev >= 2 ) then
          write(outu,'(a)') 'Turning on reading lambda from restart file'
          write(outu,'(a)') ''
       endif
       qreadlambdafromrestart = .true.
    else
       if( prnlev >= 2 ) then
          write(outu,'(a)') 'Turning off reading lambda from restart file'
          write(outu,'(a)') ''
       endif
       qreadlambdafromrestart = .false.
    endif
  end subroutine ldm_readlambdafromrestart
...
!-----------------------------------------------------------------------
!> THBV COLL <i int> <k real> <n real>
!> Adds a variable bias to thetas at site i of the form
!> Ubias = k/2*(Nhigh-1)**2 + k/2*(Nlow-Ns+1)**2
...
!> b may be replaced with AUTO to choose parameters to optimize FPL
  subroutine ldm_thetabiaspot(comlyn,comlen)
    ...
    if (INDXA(COMLYN,COMLEN,'COLL') .GT. 0) then

       if (INDXA(COMLYN,COMLEN,'RESE') .GT. 0) then
          if (qbiasthetacoll) call chmdealloc('lambda_ltm.src','ldm_thetabiaspot','ikbiasthetacoll',nsitemld,crl=ikbiasthetacoll)
          if (qbiasthetacoll) call chmdealloc('lambda_ltm.src','ldm_thetabiaspot','ipbiasthetacoll',nsitemld,crl=ipbiasthetacoll)
          qbiasthetacoll=.false.
          return
       endif

       if (.not.qbiasthetacoll) then
          if (.not. (qmld.and.qthetadm)) call wrndie(-3,'<BLOCK>','thbv must come after qldm and msld commands in block')
          call chmalloc('lambdadyn.F90','ldm_thetabiaspot','ikbiasthetacoll',nsitemld,crl=ikbiasthetacoll)
          call chmalloc('lambdadyn.F90','ldm_thetabiaspot','ipbiasthetacoll',nsitemld,crl=ipbiasthetacoll)
          ikbiasthetacoll=0
          ipbiasthetacoll=2
          qbiasthetacoll=.true.
       endif
       ...
       ipbiasthetacoll(i0:i1) = NEXTF(COMLYN,COMLEN)

    else if (INDXA(COMLYN,COMLEN,'INDE') .GT. 0) then

       if (INDXA(COMLYN,COMLEN,'RESE') .GT. 0) then
          if (qbiasthetainde) call chmdealloc('lambda_ltm.src','ldm_thetabiaspot','ikbiasthetainde',nsitemld,crl=ikbiasthetainde)
          qbiasthetainde=.false.
          return
       endif

       if (.not.qbiasthetainde) then
          if (.not. (qmld.and.qthetadm)) call wrndie(-3,'<BLOCK>','thbv must come after qldm and msld commands in block')
          call chmalloc('lambdadyn.F90','ldm_thetabiaspot','ikbiasthetainde',nsitemld,crl=ikbiasthetainde)
          ikbiasthetainde=0
          qbiasthetainde=.true.
       endif
       ...
       if (INDXA(COMLYN,COMLEN,'AUTO') .GT. 0) then
          do i = i0, i1
             b = 1
             do j = 1, 50
                ! b = 0.5 * log(b*nsubmld(i)*nsubmld(i)*pi/2)
...
!> Invoked from BLOCK subcommand CLEA
  subroutine ldm_block_clear(nblock,nreplica)
    ...
    if(qbiasthetacoll) then
       call chmdealloc('lambda_ltm.src','ldm_block_clear','ikbiasthetacoll',nsitemld,crl=ikbiasthetacoll)
       call chmdealloc('lambda_ltm.src','ldm_block_clear','ipbiasthetacoll',nsitemld,crl=ipbiasthetacoll)
    endif
    ...
    if (qldm_scalecons) then
       ...
       call chmdealloc('lambda_ltm.src','ldm_block_clear','iblcks_iat',maxaim,intg=iblcks_iat)
       call chmdealloc('lambda_ltm.src','ldm_block_clear','iblcks_ires',maxaim,intg=iblcks_ires)
    endif
    kscalecons = 0
    nires = 0
    qldm_scalecons = .false.
...
100 format("addbiasenergy> ", i5, 3f10.5)
   ...
   subroutine msld_add_restraints(nblock,x,y,z,dx,dy,dz,energy)
      use psf, only : natom
      ...
      real(chm_real) :: en
      integer :: i, j, iat
      real(chm_real) :: xcom, ycom, zcom, dr

      if (qldm_scalecons .and. kscalecons>0) then
         en = 0
         do i = 1, nires
            xcom = 0
            ycom = 0
            ...
            do j = iblcks_ires(i)+1, iblcks_ires(i+1)
               ...
               zcom = zcom + z(iat)
            enddo
            xcom = xcom / (iblcks_ires(i+1)-iblcks_ires(i))
            ycom = ycom / (iblcks_ires(i+1)-iblcks_ires(i))
            zcom = zcom / (iblcks_ires(i+1)-iblcks_ires(i))
            do j = iblcks_ires(i)+1, iblcks_ires(i+1)
               iat = iblcks_iat(j)
               dr = x(iat)-xcom
               dx(iat) = dx(iat) + kscalecons*dr
               en = en + 0.5*kscalecons*dr*dr
               dr = y(iat)-ycom
               dy(iat) = dy(iat) + kscalecons*dr
               en = en + 0.5*kscalecons*dr*dr
               dr = z(iat)-zcom
               dz(iat) = dz(iat) + kscalecons*dr
               en = en + 0.5*kscalecons*dr*dr
            enddo
         enddo
         energy = energy + en
...
! computes forces on each substituent on each site
      do i = 2, nsitemld
         ...
         do j = 1, nsubmld(i)
            ...
            fbiastheta = 0
            if (qbiasthetacoll) then
               ! en = en + 0.5*ikbiastheta*((nhigh-1)**2+(nlow-nsubmld(i)+1)**2)
               fbiastheta = fbiastheta + 0.5*2*ipbiasthetacoll(i)*0.5*ikbiasthetacoll(i) * ((nhigh-1)*(0.5*sin(thetamld(i,j))+0.5)**(ipbiasthetacoll(i)-1)*cos(thetamld(i,j)) - (nlow-nsubmld(i)+1)*(-0.5*sin(thetamld(i,j))+0.5)**(ipbiasthetacoll(i)-1)*cos(thetamld(i,j)))
            endif

            if (qbiasthetainde) then
                if (ikbiasthetainde(i) .ne. 0) then
                    ! en = en + &
                    !      ikbiastheta(i) * &
                    !      (1-(0.5-0.5*sin(thetamld(i,j)))**4)
                    fbiastheta = fbiastheta + &
                                 2 * ikbiasthetainde(i) * &
                                 (0.5 - 0.5 * sin(thetamld(i,j)))**3 * &
                                 cos(thetamld(i,j))
                endif
            endif
...
!> Restart file on unit U.
  ...
  real(chm_real) :: buffer1(nblock)
  real(chm_real) :: buffer2(nsitemld,nblock)
  character(len=128) :: line

  if (.not. qreadlambdafromrestart) return
  ...
  read(u,'(/A)',iostat=status) line
  if ( status/= 0 ) call wrndie(-3,'<lambdadyn>','EOF during read')
  buffer1(1:nblock)=bixlam(1:nblock)
  read(u,'(1D22.15)') (buffer1(i),i=1,nblock)
  if (fcnal_form=='fixd' .and. .not.all(buffer1(1:nblock).eq.bixlam(1:nblock))) &
     call wrndie(1,'<lambdadyn>','lambda in restart file differs from input script')
  bixlam(1:nblock)=buffer1(1:nblock)
  ...
  buffer2(1:nsitemld,1:nblock)=thetamld(1:nsitemld,1:nblock)
  read(u,'(1D22.15)') ((buffer2(i,j),j=1,nsubmld(i)-skip),i=2,nsitemld)
  if (fcnal_form=='fixd' .and. .not.all(buffer2(1:nsitemld,1:nblock).eq.thetamld(1:nsitemld,1:nblock))) &
     call wrndie(1,'<lambdadyn>','theta in restart file differs from input script')
  thetamld(1:nsitemld,1:nblock)=buffer2(1:nsitemld,1:nblock)
...
!-----------------------------------------------------------------------
! Adds bias on theta to energy
  subroutine msld_add_thetabiasenergy(energy)
      ...
      if (qbiasthetacoll) then
         do i = 2, nsitemld
            ...
            en = en + 0.5*ikbiasthetacoll(i)*((nhigh-1)**2+(nlow-nsubmld(i)+1)**2)
         enddo
      endif
      if (qbiasthetainde) then
         do i = 2, nsitemld
            do j = 1, nsubmld(i)
               en = en + ikbiasthetainde(i)*(1-(0.5-0.5*sin(thetamld(i,j)))**4)
            enddo
         enddo
      endif
      energy = energy + en
