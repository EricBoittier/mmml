...
float pair_vdw_force(const float r2, const float r, const float rinv,
		     const float c6, const float c12, const float c6s, const float c12s, float &pot_vdw) {
  ...
  if (vdw_model == VDW_VSH) {
    ...
    */
  } else if (vdw_model == VDW_IPS) {
       float u2 = r2*d_setup.rips2r;
       float u4 = u2*u2;
       float u6r = rinv2*rinv2*rinv2*d_setup.rips2*d_setup.rips2*d_setup.rips2;
       float u12r = u6r*u6r; 
       float dpvc=u2*(d_setup.bipsvc1+u2*(d_setup.bipsvc2+u4*d_setup.bipsvc4));
       float dpva=u2*(d_setup.bipsva1+u4*(d_setup.bipsva3 + u4*u4*d_setup.bipsva5));
       const float one_twelve = 0.0833333333333333f;
       const float one_six = 0.166666666666667f;
       fij_vdw = d_setup.rips12r*(-c12*u12r+c12s*one_twelve*dpva)-d_setup.rips6r*(-c6*u6r+c6s*one_six*dpvc);
    if (calc_energy) {
       float pvc = d_setup.aipsvc0+u2*(d_setup.aipsvc1+u2*(d_setup.aipsvc2+u4*d_setup.aipsvc4));
       float pva= d_setup.aipsva0+u2*(d_setup.aipsva1+u4*(d_setup.aipsva3  + u4*u4*d_setup.aipsva5));
       pot_vdw = one_twelve*d_setup.rips12r*(c12*u12r +c12s*pva) -one_six*d_setup.rips6r*(c6*u6r +c6s*pvc);
    }
...
float pair_elec_force(const float r2, const float r, const float rinv, 
  ...
  if (use_e14fac && elec_model != EWALD_LOOKUP && elec_model != EWALD && elec_model != ELE_IPS) {
  ...
  } else if (elec_model == ELE_IPS) {
      // Constant dielectric IPS
      float u2 =  r2*d_setup.rips2r;
      float u1r =  rinv*d_setup.rips;
      float dpe = -e14fac*u1r + u2*(d_setup.bipse1+u2*(d_setup.bipse2+u2*(d_setup.bipse3)));
      //dpe = -e14fac*u1r;
      fij_elec = qq*d_setup.ripsr*dpe;

    if (calc_energy) {
      float pe = e14fac*u1r + d_setup.aipse0 + u2*(d_setup.aipse1+u2*(d_setup.aipse2+u2*(d_setup.aipse3)));
       //pe = e14fac*u1r;
      pot_elec = qq*d_setup.ripsr*pe;
    }
...
// 1-4 exclusion force
//
template <typename AT, typename CT, int vdw_model, int elec_model, bool calc_energy, bool calc_virial, bool tex_vdwparam>
__device__ void calc_ex14_force_device(
#ifdef USE_TEXTURE_OBJECTS
				       const cudaTextureObject_t vdwParamTexObj,
#endif
                                    const int pos, const xx14list_t* ex14list,
  ...
  if (vdw_model == VDW_IPS) {
    ...
    float dpot_vdw;
    fij_vdw = pair_vdw_force<vdw_model, calc_energy>(r2, r, rinv, 0.0f, 0.0f, c6, c12, dpot_vdw);
...
template <typename AT, typename CT, int vdw_model, int elec_model, 
	  bool calc_energy, bool calc_virial, bool calc_softcore, bool tex_vdwparam>
__device__ void calc_in14_force_device(
				       ...
				       const int* vdwtype, const float* vdwparam, const float* vdwparam14,
				       const float4* xyzq, const float fscale,
				       const int stride, AT *force,
				       double &vdw_pot, double &elec_pot,
                                       double &soft_f,
...
#ifdef USE_TEXTURE_OBJECTS
    float2 c6c12 = tex1Dfetch<float2>(vdwParamTexObj, ivdw);
    float2 c6c12s = tex1Dfetch<float2>(vdwParam14TexObj, ivdw);
#else
    float2 c6c12 = tex1Dfetch(vdwparam_texref, ivdw);
    float2 c6c12s = tex1Dfetch(vdwparam14_texref, ivdw);
#endif
    c6  = c6c12.x;
    c12 = c6c12.y;
    c6s  = c6c12s.x;
  ...
  if (calc_softcore) {
    ...
    float dpot_vdw;
    CT fij_vdw = pair_vdw_force<vdw_model, calc_energy>(rp2, rp, rpinv, c6s, c12s,c6, c12, dpot_vdw);
    if (calc_energy) vdw_pot += (double)dpot_vdw;

    float dpot_elec;
    CT fij_elec = pair_elec_force<elec_model, calc_energy, true>(rp2, rp, rpinv, qq,
    ...
    float dpot_vdw;
    CT fij_vdw = pair_vdw_force<vdw_model, calc_energy>(r2, r, rinv, c6s, c12s, c6, c12, dpot_vdw);
    if (calc_energy) vdw_pot += (double)dpot_vdw;
...
#define EXPAND_ELEC(EXPAND_ENERGY_VIRIAL_NAME, KERNEL_CREATOR, KERNEL_NAME, VDW_MODEL, ...)	\
  {									\
    if (elec_model == EWALD) {					\
      ...
      EXPAND_ENERGY_VIRIAL_NAME(KERNEL_CREATOR, KERNEL_NAME, VDW_MODEL, ELE_IPS, __VA_ARGS__); \
    } else if (elec_model == NONE) {				\
...
#define CREATE_KERNELS(EXPAND_ENERGY_VIRIAL_NAME, KERNEL_CREATOR, KERNEL_NAME, ...) \
  {									\
    if (vdw_model == VDW_VSH) {					\
      ...
      EXPAND_ELEC(EXPAND_ENERGY_VIRIAL_NAME, KERNEL_CREATOR, KERNEL_NAME, VDW_IPS, __VA_ARGS__); \
    } else {								\
...
#ifdef USE_TEXTURE_OBJECTS
      CREATE_KERNELS(EXPAND_ENERGY_VIRIAL, CREATE_KERNEL, calcForceBlockSCKernel, vdwParamTexObj,
		     base, nlist.get_n_ientry(), nlist.get_ientry(), nlist.get_tile_indj(),
		     nlist.get_tile_excl(), stride, vdwparam, nvdwparam, xyzq, vdwtype,
		     ...
		     force, virial, energy_vdw, energy_elec);
#else
      CREATE_KERNELS(EXPAND_ENERGY_VIRIAL, CREATE_KERNEL, calcForceBlockSCKernel,
		     base, nlist.get_n_ientry(), nlist.get_ientry(), nlist.get_tile_indj(),
		     nlist.get_tile_excl(), stride, vdwparam, nvdwparam, xyzq, vdwtype,
		     cudaBlock->getNumBlock(), cudaBlock->getBixlam(), cudaBlock->getBlockType(),
		     biflam, biflam2,
...
void calcForce14KernelChoice(const int nblock, const int nthread, const int shmem_size, cudaStream_t stream,
			     ...
			     const int* in14BlockToTblPos, const int* ex14BlockToTblPos,
                             const int* vdwtype, const float* vdwparam, const float* vdwparam14,
#ifdef USE_TEXTURE_OBJECTS
			     cudaTextureObject_t& vdwParamTexObj, cudaTextureObject_t& vdwParam14TexObj,
#endif
			     const float4* xyzq, const float fscale,
                             const int stride, AT* force,
  ...
  if (cudaBlock == NULL) {
#ifdef USE_TEXTURE_OBJECTS
  CREATE_KERNELS(EXPAND_ENERGY_VIRIAL, CREATE_KERNEL14, calc_14_force_kernel, vdwParamTexObj, vdwParam14TexObj,
		 nin14list, nex14list, nin14block, in14list, ex14list,
		 vdwtype, vdwparam, vdwparam14, xyzq, fscale, stride, force, virial, energy_vdw, energy_elec, energy_excl);
#else
...
#ifdef USE_TEXTURE_OBJECTS
  CREATE_KERNELS(EXPAND_ENERGY_VIRIAL, CREATE_KERNEL14, calc_14_force_block_sc_kernel, vdwParamTexObj, vdwParam14TexObj,
		 nin14list, nex14list, nin14block, in14list, ex14list,
                 in14TblBlockPos, ex14TblBlockPos, in14BlockToBlock,
                 ex14BlockToBlock, in14BlockToTblPos, ex14BlockToTblPos,
		 vdwtype, vdwparam, vdwparam14, xyzq,
                 *(cudaBlock->getBlockParamTexObj()),
                 cudaBlock->getUsePMEL(),
                 cudaBlock->getBlockParamEx(),
                 cudaBlock->getDSoftDFscale(),
                 stride, force, virial, energy_vdw, energy_elec, energy_excl);
#else
  CREATE_KERNELS(EXPAND_ENERGY_VIRIAL, CREATE_KERNEL14, calc_14_force_block_sc_kernel,
		 nin14list, nex14list, nin14block, in14list, ex14list,
                 in14TblBlockPos, ex14TblBlockPos, in14BlockToBlock,
                 ex14BlockToBlock, in14BlockToTblPos, ex14BlockToTblPos,
		 vdwtype, vdwparam, vdwparam14, xyzq,
...
#ifdef USE_TEXTURE_OBJECTS
  CREATE_KERNELS(EXPAND_ENERGY_VIRIAL, CREATE_KERNEL14, calc_14_force_block_kernel, vdwParamTexObj, vdwParam14TexObj,
		 nin14list, nex14list, nin14block, in14list, ex14list,
                 in14TblBlockPos, ex14TblBlockPos, in14BlockToBlock,
                 ex14BlockToBlock, in14BlockToTblPos, ex14BlockToTblPos,
		 vdwtype, vdwparam, vdwparam14, xyzq,
                 *(cudaBlock->getBlockParamTexObj()),
                 ...
                 stride, force, virial, energy_vdw, energy_elec, energy_excl);
#else
  CREATE_KERNELS(EXPAND_ENERGY_VIRIAL, CREATE_KERNEL14, calc_14_force_block_kernel,
		 nin14list, nex14list, nin14block, in14list, ex14list,
                 in14TblBlockPos, ex14TblBlockPos, in14BlockToBlock,
                 ex14BlockToBlock, in14BlockToTblPos, ex14BlockToTblPos,
		 vdwtype, vdwparam, vdwparam14, xyzq,
