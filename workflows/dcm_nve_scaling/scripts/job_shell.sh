...
N_MONOMERS="${1:?usage: job_shell.sh N_MONOMERS}"
...
echo "=== dcm_nve_scaling: DCM:${N_MONOMERS} ===" >&2
...
exec "$PY" "$WORKFLOW_ROOT/scripts/run_job.py" "$N_MONOMERS" --config "$WORKFLOW_ROOT/config.yaml"
