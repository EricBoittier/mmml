"""Health monitoring helpers for pbc_solvent_burst campaigns."""
...
_LOG_ACTIVITY = re.compile(
    r"MLpot SD minimize|heat segment|DYNA>|JAX-MD|Running:|Campaign jobs",
    re.IGNORECASE,
)
...
def _classify_status(
    *,
    ...
    if grep_errors(log_text):
        if "burst campaign failed" in log_text.lower() or "segmentation fault" in log_text.lower():
            return "failed"
