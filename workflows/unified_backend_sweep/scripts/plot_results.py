...
fluctuation (std) and tendency (linear trend) -- see
...
failure) are marked "FAILED" instead of a bar, since they have no numeric
trace to plot.

Uses ``mmml.utils.plotting.styles`` (the house style module) for
fonts/colors instead of an ad-hoc palette.
"""
...
_STYLE_NAME = "nature"
_BACKENDS = ("jaxmd_min", "jaxmd_nve", "jaxmd_nvt", "jaxmd_npt", "rigid_mc")
...
def plot_summary_bars(rows: list[dict[str, str]], out: Path, style) -> Path:
    rows = sorted(rows, key=lambda r: (r["backend"], int(r["seed"])))
    labels = [f"{row['backend']}\n(seed {row['seed']})" for row in rows]
    colors_by_backend = dict(zip(_BACKENDS, comparison_colors(style, len(_BACKENDS))))
    colors = [colors_by_backend.get(row["backend"], "#999999") for row in rows]

    fig, (ax_fluct, ax_trend, ax_time) = plt.subplots(3, 1, figsize=(10, 8), sharex=True)

    for i, row in enumerate(rows):
        if row["completed"] == "True":
            fluctuation, trend = _fluctuation_and_trend(row)
            ax_fluct.bar(i, fluctuation, color=colors[i])
            ax_trend.bar(i, trend, color=colors[i])
        else:
    ...
    handles = [Patch(color=c, label=b) for b, c in colors_by_backend.items()]
    ax_fluct.legend(handles=handles, loc="best", fontsize=8, ncol=2)
...
def main() -> None:
    ...
    bars_path = plot_summary_bars(rows, out_dir / "summary_bars.png", style)
