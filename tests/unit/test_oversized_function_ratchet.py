...
_BASELINE: dict[str, int] = {
    # 3363 -> 3445 -> 3438 -> 3702. Bonded-intra damping was call sites only
    # (logic at module level as `apply_bonded_intra_damping` /
    # `bonded_intra_bundle`). Metatomic MM-only wiring then grew it past the
    # grace; that block is now `resolve_hybrid_ml_backend_mode`,
    # `metatomic_mm_only_model_bundle`, and the dashboard helpers. The jump to
...
#
# 27 -> 28: `_build_jax_decomposed_mlpot_model` crossed 500 (now 509) with the
# one-Hamiltonian / live-box work ahead of #266. Recorded rather than split
# in this CI fix; still a candidate for extraction.
_MAX_CROWDED = 28
